  ...
  const footer = new Footer({
    children: [
      ...
      new Paragraph({
        tabStops: rightStop,
        children: [
          new TextRun({ text: legalFooterLine(), size: 13, color: FAINT }),
          footerTab(),
          new TextRun({ text: 'Prepared with ', size: 13, color: FAINT }),
          // The Route W wordmark is a weight contrast, not a colour one -
          // light "Work" against a heavier "Fleet", both in graphite.
          new TextRun({ text: 'Work', size: 13, color: PLATFORM_INK }),
          new TextRun({ text: 'Fleet', size: 13, color: PLATFORM_INK, bold: true }),
        ],
      }),
    ],
  });
