...
import { ROOM_TYPES, ADDON_TYPES, GARDEN_ADDON_TYPES, OVEN_OPTIONS, STANDARD_INCLUSIONS, QUOTE_SEPARATELY_ITEMS, pricePeriodSuffix, projectPricePeriods } from './quoteCalculator';
...
// visitsPerWeek is a rate, not a count: fortnightly is 0.5 and monthly
// is 12/52. Printed literally that put "0.23076923076923078 visits per
// week" on a client's quotation, so it's expressed over the shortest
// period that makes it a whole visit instead.
function describeVisitPattern(visitsPerWeek) {
  const perWeek = Number(visitsPerWeek);
  if (!Number.isFinite(perWeek) || perWeek <= 0) return null;

  const [count, period] =
    perWeek >= 1 ? [perWeek, 'week']
    : perWeek >= 0.5 ? [perWeek * 2, 'fortnight']
    : [perWeek * (52 / 12), 'month'];

  const visits = Math.round(count * 10) / 10;
  return `${visits} visit${visits === 1 ? '' : 's'} per ${period}`;
}
...
export function quoteSummaryLines(quote, company = COMPANY) {
  ...
  if (schedule) {
    ...
    const pattern = describeVisitPattern(b?.visitsPerWeek);
    if (pattern) lines.push({ label: 'Proposed pattern', value: pattern });
  }
...
function pricingSection(quote, meta, schedule, { compact = false } = {}) {
  const b = quote.calculator_breakdown;
  const blocks = [{ type: 'price', label: 'Quoted price', value: quotedPriceText(quote) }];
  ...
  if (b) {
    // The hours the client is billed for, which is not always the hours
    // we expect to be on site: a minimum callout or the minimum job
    // price can set the charge above the estimate. Pricing a line off
    // the estimate then prints a rate nobody is paying and a row that
    // doesn't multiply out - "2.5 hours @ £48.00 per hour - £120.00".
    // Older breakdowns predate chargeableHours, hence the fallback.
    const chargedHours = Number(b.chargeableHours) || Number(b.totalHours) || 0;
    const hours = formatHours(chargedHours);
    const rows = [];

    const ovenCharge = Number(b.ovenCharge) || 0;
    const labourPrice = Number(quote.price) - ovenCharge;
    const rate = chargedHours > 0 ? formatPriceGBP(labourPrice / chargedHours) : null;
...
// The price as printed on the document - "£180.00 per week" rather
// than a number the client has to guess the unit of.
function quotedPriceText(quote) {
  return `${formatPriceGBP(quote.price)}${pricePeriodSuffix(quote.price_period)}`;
}

function periodProjection(quote) {
  return projectPricePeriods(quote.price, quote.price_period);
}

function contractValueSection(quote) {
  const b = quote.calculator_breakdown;

  // A price typed per week, month or year projects into the other
  // periods without needing a visit pattern.
  const projected = periodProjection(quote);
  if (projected && !b?.weeklyContractValue) {
    return {
      title: 'Contract Value',
      blocks: [
        { type: 'table', columns: ['Period', 'Value'], rows: [
          ['Per week', formatPriceGBP(projected.weekly)],
          ['Per month', formatPriceGBP(projected.monthly)],
          ['Per year', formatPriceGBP(projected.annual)],
        ] },
        { type: 'para', text: 'Indicative, based on the quoted price. Additional attendances are charged separately.' },
      ],
    };
  }
...
function acceptanceSection(quote, meta, schedule, company) {
  const b = quote.calculator_breakdown;
  const items = [{ label: 'Quoted price', value: quotedPriceText(quote) }];
  if (schedule) {
  ...
  } else if (periodProjection(quote)) {
    const p = periodProjection(quote);
    items.push({ label: 'Weekly value', value: formatPriceGBP(p.weekly) });
    items.push({ label: 'Monthly value', value: formatPriceGBP(p.monthly) });
    items.push({ label: 'Annual value', value: formatPriceGBP(p.annual) });
  }
