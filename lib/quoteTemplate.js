...
export function quoteSections(quote, company = COMPANY) {
  const meta = quoteDocumentMeta(quote);
  const schedule = summariseShiftSchedule(quote.shift_schedule);
  ...
  if (!isContractQuote(quote)) {
    return [
      ...
      contractValueSection(quote),
      shortAcceptanceSection(quote, meta, company),
    ].filter(Boolean).map((section, i) => ({ ...section, number: i + 1 }));
  }

  const sections = [
    introSection(quote, meta, company),
    scopeSection(quote),
    ...
    staffingSection(company),
    isCommercial ? ramsSection(company) : null,
    meta.serviceType === 'gardening' ? null : coshhSection(company),
    healthAndSafetySection(company),
    qualitySection(company),
    pricingSection(quote, meta, schedule),
    schedule ? scheduleContractValueSection(schedule) : contractValueSection(quote),
    materialsSection(company),
    contractReviewSection(quote, schedule, company),
    clientRequirementsSection(company),
    acceptanceSection(quote, meta, schedule, company),
  ].filter(Boolean);
