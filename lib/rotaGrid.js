...
export function buildCleanerRows(jobs, cleaners, weekDays) {
  ...
  (jobs || []).forEach((job) => {
    ...
    targets.forEach((id) => {
      ...
      row.jobCount += 1;
      row.minutes += shareMinutes(job, id === UNASSIGNED_ROW_ID ? null : id);
    });
  });
...
// One person's minutes on a job, for their hours column. A job with two
// people on it is split between them - an 8-hour job with two cleaners is
// 4 hours each - unless the office has set that person's minutes by hand
// (job_assignments.paid_minutes), the same rule as lib/hoursWorked.js and
// assignment_paid_minutes() in the database, so the rota agrees with the
// cleaner's own hours page and payroll. With no cleaner (the "needs a
// cleaner" row) the whole job is what is waiting to be covered.
export function shareMinutes(job, cleanerId) {
  const full = job.duration_minutes || DEFAULT_DURATION_MINUTES;
  const people = (job.job_assignments || []).filter((a) => a && a.cleaner_id);
  if (!cleanerId || people.length === 0) return full;
  const mine = people.find((a) => a.cleaner_id === cleanerId);
  if (mine?.paid_minutes != null) return Number(mine.paid_minutes);
  return full / people.length;
}
