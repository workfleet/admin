...
const styles = StyleSheet.create({
  ...
  // Colour applied at render time, not here - the brand colour is an
  // editable setting and a StyleSheet is built once at import.
...
function Block({ block, company }) {
  switch (block.type) {
    ...
    case 'signature':
      return <SignatureBlock company={company} />;
    default:
      return null;
  }
}

export default function QuotePdfDocument({ quote, company = COMPANY }) {
  const meta = quoteDocumentMeta(quote);
  const sections = quoteSections(quote, company);
