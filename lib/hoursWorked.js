...
// The hours a cleaner is booked for between two dates (inclusive, local
// calendar days), for filling in a holiday request: what they would have
// worked is what the holiday replaces. Only shifts still to come count -
// a day already worked is not one anyone is taking off. Rounded to the
// nearest half hour, the step the request form uses.
export function bookedHoursBetween(jobs, assigneeCounts, startDate, endDate, now = new Date()) {
  if (!startDate || !endDate || endDate < startDate) return 0;
  const total = jobs
    .filter((j) => j.status === 'scheduled' && new Date(j.scheduled_at) > now)
    .filter((j) => {
      const key = localDateString(new Date(j.scheduled_at));
