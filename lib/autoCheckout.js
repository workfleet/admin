...
import { distanceMeters, geofenceRadiusFor } from './geo';
...
// while they stood still, so 100m gets you on site and it takes 200m to be
// counted as having left. This constant is the floor for a property with a
// small custom radius; the default property's line is double its radius.
...
export function classifyFix(fix, property) {
  ...
  // A property with its own, larger radius (0089) keeps the same shape: the
  // "left" line sits at double the check-in radius, never closer than the
  // default 150 m, so the uncertain band scales with the site.
  const inner = geofenceRadiusFor(property);
  const outer = Math.max(AUTO_CHECKOUT_RADIUS_METERS, inner * 2);
  const distance = distanceMeters(fix.lat, fix.lng, property.lat, property.lng);
  if (distance <= inner) return 'inside';
...
// When a shift is expected to be over, given when they actually started.
// The booked duration is the promise, not the booked end: someone who
// clocked in an hour late is still working an hour after the booked end,
// and closing them at the booked end would clock them short (0098). An
// early arrival keeps the booked end, which is the later of the two.
export function expectedShiftEnd(job, checkin, now) {
  if (!job?.scheduled_at) return now;
  const duration = (job.duration_minutes || 0) * 60000;
  const bookedEnd = new Date(job.scheduled_at).getTime() + duration;
  const arrived = checkin?.checked_in_at ? new Date(checkin.checked_in_at).getTime() : bookedEnd;
  return new Date(Math.max(bookedEnd, arrived + duration));
}
...
//     falls back to the end of their booked time, counted from when they
//     clocked in - the same assumption reconcile_job_statuses() makes.
//
...
export function autoCheckoutTimestamp({ observedDepartureAt, checkin, job, now = new Date() }) {
  ...
  const candidate = observedDepartureAt
    ...
    // booked until.
    : new Date(Math.max(expectedShiftEnd(job, checkin, now).getTime(), lastInside));
