...
// What a quote's price is for. A one-off clean is priced once; a
// contract is priced by the period it is billed on. Kept next to the
// price rather than inferred from the description, because "£180" and
// "£180 a week" are different quotes and the client's document has to
// say which.
export const PRICE_PERIODS = [
  { value: 'one_off', label: 'One-off price', suffix: '' },
  { value: 'visit', label: 'Per visit', suffix: ' per visit' },
  { value: 'week', label: 'Per week', suffix: ' per week' },
  { value: 'month', label: 'Per month', suffix: ' per month' },
  { value: 'year', label: 'Per year', suffix: ' per year' },
];

// Weeks in a month is 52/12 throughout, matching the contract-value
// projection above - a monthly price is not four weeks of the weekly one.
const WEEKS_PER_YEAR = 52;
const MONTHS_PER_YEAR = 12;

export function pricePeriodSuffix(period) {
  return PRICE_PERIODS.find((p) => p.value === period)?.suffix ?? '';
}

// The same price expressed per week, per month and per year, so the
// office can type whichever figure the client asked for and still see
// the annual value before sending. null for a one-off or per-visit
// price - neither has a period to project without knowing how often
// the visits happen (the commercial calculator handles per-visit).
export function projectPricePeriods(price, period) {
  if (!isUsableNumber(price)) return null;
  const p = Number(price);

  let annual;
  if (period === 'week') annual = p * WEEKS_PER_YEAR;
  else if (period === 'month') annual = p * MONTHS_PER_YEAR;
  else if (period === 'year') annual = p;
  else return null;

  return {
    weekly: round2(annual / WEEKS_PER_YEAR),
    monthly: round2(annual / MONTHS_PER_YEAR),
