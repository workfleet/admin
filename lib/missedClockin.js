...
// figure is right.
export function unpaidMissedJobs(jobs, claims = [], now = new Date(), outcomes = []) {
  ...
  // A shift the office has already accounted for (missed_shift_outcomes,
  // 0093 - client cancelled, off sick, and so on) is not quietly missing
  // either: someone has looked at it and said why it is unpaid. It is not
  // worth nagging them to claim a visit the client called off. The job page
  // still takes a claim if they disagree with what was recorded.
  outcomes.forEach((o) => spokenFor.add(o.job_id));
  return jobs
