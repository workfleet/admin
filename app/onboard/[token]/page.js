...
import { buildContractText, contractTerms } from '../../../lib/staffContract';
...
export default function OnboardPage() {
  ...
  // The terms of this particular offer, set by the office on the invite.
  // Shown, never edited here - the submit route builds the signed text from
  // its own copy, so anything changed in the browser would be thrown away.
  ...
  useEffect(() => {
    fetch(`/api/onboarding/${token}`)
      .then(async (res) => {
        ...
        // Whatever the office already knew is filled in for them, and stays
        // editable - a prefilled address they can correct beats an empty box
        // on a phone keyboard.
        setForm((f) => ({
          ...f,
          full_name: body.expected_name || '',
          email: body.email || '',
          address: body.expected_address || '',
          phone: body.expected_phone || '',
          date_of_birth: body.expected_date_of_birth || '',
        }));
  ...
  return (
    <div className="container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="card">
          <h2>Contract</h2>
          {/* The terms that differ from hire to hire, out where they can be
              read, rather than only inside clauses 1.4, 1.7 and 6.1. */}
          <div style={{ fontSize: 13.5, lineHeight: 1.7, marginTop: -2, marginBottom: 12 }}>
            <div className="task-row"><strong style={{ width: 110, flexShrink: 0 }}>Role</strong><span>{terms.jobTitle}</span></div>
            <div className="task-row">
              <strong style={{ width: 110, flexShrink: 0 }}>Pay</strong>
              <span>£{terms.hourlyRate} per hour, paid {terms.payFrequency}</span>
            </div>
            {terms.startDate && (
              <div className="task-row"><strong style={{ width: 110, flexShrink: 0 }}>Start date</strong><span>{terms.startDate}</span></div>
            )}
            {terms.reportsTo && (
              <div className="task-row"><strong style={{ width: 110, flexShrink: 0 }}>Reporting to</strong><span>{terms.reportsTo}</span></div>
            )}
          </div>
          <div
            style={{
